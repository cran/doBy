...
\usepackage{inputenc,hyperref,a4wide,color,boxedminipage,Sweave}
...
working document describes some of these functions. 
...
The \doby\ package
(and this document as a .pdf file) is available from
...
calulated with \verbålsmeans()å. See the documentation of  \verbålsmeans()å for
examples. 
